...
class CareerTrackState extends StatefulWidget {
  const CareerTrackState({Key? key}) : super(key: key);
  
  @override
  _CareerTrackState createState() => _CareerTrackState();
}

class _CareerTrackState extends State<CareerTrackState> {
  ...
  void initState() {
    ...
    utils.getToken()!.then((value) {
      ...
      _getUserInfo()!.then((record) {
        ...
        _bloc!.loadCareerTrackList(token: _token, myKad: _user!['identidyId']);
      });
    ...
    return AnnotatedRegion<SystemUiOverlayStyle>(
      ...
      child: Scaffold(
        ...
        body: RefreshIndicator(
          color: mainColor,
          onRefresh: () {
            return _bloc!.loadCareerTrackList(token: _token, myKad: _user!['identidyId']);
          },
          child: StreamBuilder<Response<CareerTrackListModel?>>(
              ...
              if (snapshot.hasData) {
                switch (snapshot.data!.status!) {
                  ...
                  case Status.completed:
                    return snapshot.data!.data!.careerTrackList!.isNotEmpty ? ListView.builder(
                      physics: const AlwaysScrollableScrollPhysics(),
                      controller: scrollController,
                      shrinkWrap: true,
                      itemCount: snapshot.data!.data!.careerTrackList!.length,
                      itemBuilder: (BuildContext context, int index) => _informationItem(snapshot.data!.data!.careerTrackList![index])
                    ) : utils.noDataFound(context);
                  case Status.error:
                    return ErrorSync(
                      errorMessage: snapshot.data!.message,
                      onRetryPressed: () => _bloc!.loadCareerTrackList(token: _token, myKad: _user!['identidyId'])
                    );
                }
              }
              return Container();
  ...
  Widget _informationItem(CareerTrackInfoModel item) {
    return Container(
                        ...
                        Container(
                          ...
                          child: Material(
                            ...
                            child: InkWell(
                              ...
                              onTap: () => {
                                Navigator.of(context, rootNavigator: true).push(MaterialPageRoute(builder: (context) => CareerTrackDetailsState(
                                  applyJobNo: item.no!
                                )))
                              }
                            )
